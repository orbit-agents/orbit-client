...
interface ShapeGridProps {
  ...
  /** Frame-rate cap. Default 30 fps — backdrop quality is plenty. */
  maxFps?: number;
  /** Pause render loop when canvas is off-screen. Default true. */
  suspendWhenOffscreen?: boolean;
  /** Color the vignette fades into at the corners. Default `#120F17`. */
  vignetteColor?: string;
  /** Optional className passed to the wrapping canvas element. */
  className?: string;
}
...
const ShapeGrid: React.FC<ShapeGridProps> = ({
  ...
  useEffect(() => {
    ...
    if (reducedMotion) {
      // Render once and freeze; honor prefers-reduced-motion.
      drawGrid();
    } else {
      start();
    }
  ...
  return (
    <canvas
      ref={canvasRef}
      className={className ?? "w-full h-full border-none block"}
    />
  );
};
